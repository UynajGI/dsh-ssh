/**
 * The 「新建远程」form: one SSH connection with password or private-key
 * authentication (二选一), an optional ProxyJump chain, and `~/.ssh/config`
 * alias recognition through the Host channel. Field-level validation with
 * typed feedback (info / success / error).
 */
...
export function ConnectionForm({ resolve, test, save, onClose, onSaved }: ConnectionFormProps) {
  ...
  const errors = (): FieldErrors => {
    ...
    if (host.trim() === '') result.host = '请填写主机地址或 ~/.ssh/config 别名'
  ...
  const resolveConfig = async (): Promise<void> => {
    setRevealed(true)
    if (host.trim() === '') {
      setFeedback({ kind: 'error', text: '请先填写主机地址（或 ~/.ssh/config 别名）' })
      return
    }
    setBusy(true)
    setBusyTask('resolve')
    ...
    try {
      ...
      const jump = resolved.jump.map(hop =>
        `${hop.username !== undefined && hop.username !== '' ? `${hop.username}@` : ''}${hop.host}${hop.port !== undefined && hop.port !== 22 ? `:${String(hop.port)}` : ''}`)
      setJumpText(jump.join(', '))
  ...
  const runTest = async (): Promise<void> => {
    ...
    if (host.trim() === '') {
      setFeedback({ kind: 'error', text: '请先填写主机地址' })
      return
    }
      ...
      <div className={styles.form} role="dialog" aria-modal="true" aria-label="新建远程连接" ref={dialogRef}>
        <div className={styles.formHead}>
          <div className={styles.formHeadText}>
            ...
            <p className={styles.formSub}>保存后将出现在连接列表中，可直接浏览其远程目录</p>
          </div>
        ...
        <div className={styles.formGrid}>
          <label className={styles.field}>
            <span className={styles.fieldLabel}>主机地址 / 别名<span className={styles.required} aria-hidden>*</span></span>
            <input
              className={cx(styles.input, hostError !== undefined && styles.inputError)}
              value={host}
              placeholder="172.28.17.81 或 ssh config 别名"
              disabled={busy}
          ...
          <label className={styles.field}>
            ...
            <input
              className={styles.input}
              value={jumpText}
              placeholder="user@host:port，多台用逗号分隔"
              disabled={busy}
              onChange={event => { setJumpText(event.target.value) }}
            />
            <span className={styles.fieldHint}>格式 user@host:port，按连接顺序排列，支持多级跳板</span>
          </label>
        </div>
